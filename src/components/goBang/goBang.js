...
GoBang.prototype = {
  ...
  handlerTouch: function (e) {
    switch (e.type) {
      ...
      this.touch.x2 = this.touch.x1 = e.clientX || e.touches[0].clientX;
      this.touch.y2 = this.touch.y1 = e.clientY || e.touches[0].clientY;
      break;
      case "touchmove": case "mousemove":
      if (this.isTouch) {
        this.touch.end = new Date().getTime();
        if (this.touch.end - this.touch.start < 300) {
          this.touch.x2 = e.clientX || e.touches[0].clientX, this.touch.y2 = e.clientY || e.touches[0].clientY;
        }
        else {
          this.isTouch = false;
        }
